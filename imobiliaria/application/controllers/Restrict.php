...
class Restrict extends CI_Controller {
    ...
    public function ajax_login(){
        ...
        $password_in = $this->input->post("password");

        if (empty($username)) {
            $json["status"] = 0;
            $json["error_list"]["#username"] = "Usuario não pode estar vazio!";
        } else {
        ...
        if ($result) {
                ...
                if ($json["status"] == 0) {
                    $json["error_list"]["#btn_login"] = "Usuário ou senha incorreta!";
                }
            }
    ...
    public function ajax_save_course(){
        ...
        if (empty($data["course_duration"])) {
            ...
            if (!($data["course_duration"] > 0 && $data["course_duration"] < 100)) {
                $json["error_list"]["#course_duration"] = "Duração do curso deve ser maior que 0 Hrs e menor que 100 Hrs";
            }
        }
        ...
        if (empty($data["user_email"])) {
            ...
            if ($this->users_model->is_duplicated("user_email", $data["user_email"], $data["user_id"])) {
                ...
                if($data["user_email"] != $data["user_email_confirm"]){
                    ...
                    $json["error_list"]["#user_email_confirm"] = "Os E-mails digitados não conferem!";
                }
            }
        }
        ...
        if (empty($data["user_password"])) {
            $json["error_list"]["#user_password"] = "Senha é obrigatório!";
                ...
                if($data["user_password"] != $data["user_password_confirm"]){
                    ...
                    $json["error_list"]["#user_password_confirm"] = "As senhas digitadas não conferem!";
                }
        }
